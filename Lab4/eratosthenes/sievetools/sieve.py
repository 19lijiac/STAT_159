...
def proportion_primes(nmax_lower, nmax_upper, increment):
    
    """
    proportion of prime numbers smaller than some `nmax`, starting from nmax_lower to nmax_upper with the 
    specified increment.
  
    Parameters:
    arg1 (int): the lower bound for nmax
    arg2 (int): the upper bound for nmax
    increment (int): differences/steps between each nmax
  
    Returns:
    list: proportion of prime numbers smaller than some `nmax`
    """
    
    all_nmax = np.arange(nmax_lower, nmax_upper, increment)
    all_proportions = []
